...
version is available. The immutable `v0.1.0` GitHub release failed its
production-only CircleCI review before registry publication. Source examples
reference the planned `vexcalibur-dev/vexcalibur@0.1.1` patch release. Do not
import the development reference into a project with environment variables,
contexts, private source, or other credentials.
...
| Planned production orb version | `0.1.1` |
...
| `--csaf-publisher-namespace` | `https://security.example.com` | Absolute URL controlled by the publisher. |
| `--csaf-publisher-category` | `vendor` | One of `coordinator`, `discoverer`, `other`, `user`, or `vendor`. |
| `--csaf-document-status` | `final` | One of `draft`, `final`, or `interim`; the CLI default is `draft`. |

Direct CSAF file output must use the filename derived from the tracking ID. The example ID becomes `example-csaf-vex-2026-001.json`, so the example writes `artifacts/example-csaf-vex-2026-001.json`.

The sample `in_triage` finding needs no extra state evidence. Other states may require an action, impact, remediation category, or fixed version. The [CSAF output reference](https://vexcalibur-dev.github.io/vexcalibur/reference/csaf-output.html) defines those mappings and the filename algorithm.

This integration generates CSAF. It does not read CSAF, convert existing VEX documents, sign output, or publish a trusted-provider feed.
