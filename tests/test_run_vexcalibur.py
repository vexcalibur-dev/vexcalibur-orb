...
class RunVexcaliburScriptTests(unittest.TestCase):
    ...
    def test_rejects_non_exact_release_by_default(self) -> None:
        env = base_env()
        env["VEXCALIBUR_ORB_PACKAGE_SPEC"] = "vexcalibur>=0.2.0"
    ...
    def test_rejects_prerelease_by_default(self) -> None:
        env = base_env()
        env["VEXCALIBUR_ORB_PACKAGE_SPEC"] = "vexcalibur==0.2.0.dev1"
            ...
            env.update(
                {
                    ...
                    "VEXCALIBUR_ORB_PACKAGE_SPEC": "vexcalibur==0.2.0",
    ...
    def test_constraints_file_is_passed_to_pip_install(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertIn(
                f"-I -m pip --isolated --no-cache-dir install --constraint {constraints_file} vexcalibur==0.2.0",
